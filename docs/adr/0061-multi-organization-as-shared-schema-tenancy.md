...
14. **The active organization travels in a request header, not in the token.**
    ...
    - Header absent and the user has exactly one membership: use it. Every
      ...
      exists.
    - Header absent and the user has several: refuse with a defined code, in the
      shape `terms_not_accepted` already uses, which the app answers by opening
      the switcher. The ambiguous case fails closed rather than guessing an
      organization on someone's behalf.
...
16. **One panel, not two.** `platform_admin` can do everything an organization
    admin can do, plus create, read, update and delete organizations, and must
    ...
    decision 13 creates.
17. **Terms, acceptance state and notifications belong to one organization.**
...
18. **The organization is always passed in, never discovered.** Decision 14
...
19. **Groups are isolated data, not the isolation boundary.** Groups are
...
20. **The IoT contract does not change, and the simulator is the evidence.**
