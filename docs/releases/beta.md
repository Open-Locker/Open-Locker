...
sits in an open pull request. Work that exists only as an issue is out of scope.
...
**`locker-client/.simulator-credentials.json.bak`.** Sitting untracked in the working
tree right now. Confirm it is gitignored and delete it — a credentials backup is not
