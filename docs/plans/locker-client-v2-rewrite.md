...
Living document for the parallel rewrite. Architecture decision: [ADR-0027](../adr/0024-locker-client-v2-hexagonal-rewrite.md).
...
| Hexagonal layer discipline | Manual ports/adapters + ADR-0027 | **Partial (CI only)** — `boundaries.preset: "hexagonal"` could gate import violations in PRs, but does not replace runtime design |
| Lost requests under load | Priority queue + serialized bus actor | **No** |

`p-queue` + `WaveshareModbusBusActor` directly address ADR-0027's concurrency requirements. Fallow operates in a completely orthogonal layer (developer tooling).
...
| Team stack | TypeScript service; adding Rust tooling to the **runtime** path conflicts with ADR-0027 (explicit TS rewrite, rejected Rust rewrite for delivery reasons) |
...
| Option | Pros | Cons |
| --- | --- | --- |
| **Reject (chosen)** | Keeps runtime deps minimal per ADR-0027; no confusion between static analysis and hardware bridge; `p-queue` + `BusActor` already solve serialization | No automated boundary enforcement in CI |
...
- **Runtime:** **No.** Continue with `WaveshareModbusBusActor` + `p-queue` + `CommandDispatcher` as decided in ADR-0027.
...
New sessions: read this file + ADR-0027 before editing `locker-client/`.
