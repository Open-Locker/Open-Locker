...
class Locker {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is Locker &&
    other.id == id &&
    other.name == name &&
    other.isOpen == isOpen;
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "Locker[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "Locker[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<Locker> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <Locker>[];
  ...
  static Map<String, List<Locker>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<Locker>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = Locker.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
