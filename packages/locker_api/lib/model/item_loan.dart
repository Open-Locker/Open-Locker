...
class ItemLoan {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is ItemLoan &&
    other.id == id &&
    other.item == item &&
    other.userId == userId &&
  ...
  @override
  String toString() => 'ItemLoan[id=$id, item=$item, userId=$userId, borrowedAt=$borrowedAt, returnedAt=$returnedAt]';
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "ItemLoan[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "ItemLoan[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<ItemLoan> listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<ItemLoan>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<ItemLoan>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = ItemLoan.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
