...
describe('baseQuery organization handling', () => {
  ...
  it('asks for a selection when the server refuses to guess', async () => {
    mockFetchWithBody(409, { code: 'organization_not_selected' });

    await dispatchProbe('ambiguous');

    expect(store.getState().organization.selectionRequired).toBe(true);
  });
  ...
  it('drops a stored organization the user may not act in', async () => {
    ...
    expect(store.getState().organization.selectionRequired).toBe(false);
  });
});
